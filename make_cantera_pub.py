#!/usr/bin/env python3
"""Publica CANTERA: m4a del set (ya masterizado, sin loudnorm), cantera.js, cortes standalone."""
import os, json, subprocess
import numpy as np, imageio_ffmpeg
from dream_core import ffmeter, master_file
from make_cantera import SECTIONS, TMP, SPB, XF_BARS

FF = imageio_ffmpeg.get_ffmpeg_exe()
HERE = os.path.dirname(os.path.abspath(__file__))
SRC = os.path.join(HERE, 'masters', 'amr-cantera.wav')
CUTS = ['BLOQUE', 'CINCEL', 'TALLER', 'CATEDRAL', 'CAMPANAS', 'PLAZA']
...
if __name__ == '__main__':
    # 1. set completo
    m4a = os.path.join(HERE, 'audio', 'amr-cantera.m4a')
    enc(SRC, m4a)
    ...
    meta = dict(id='amr-cantera', title='CANTERA', kicker='ROSA CANTERA · 0→100',
                tracks=len(titles), dur=round(acc, 1), titles=titles, offsets=offsets,
                file='audio/amr-cantera.m4a', art='art/shots/shot-cantera.svg', edition=12,
                peaks=peaks(m4a), bpm=125, key='A MIN')
    with open(os.path.join(HERE, 'cantera.js'), 'w') as f:
        f.write('window.AMR_CANTERA=' + json.dumps(meta) + ';')
    print('cantera.js escrito', flush=True)
    ...
    for i, s in enumerate(SECTIONS):
        ...
        dst = os.path.join(HERE, 'audio', f'amr-cantera-cut-{s["name"].lower()}.m4a')
        enc(mst, dst)
        os.remove(mst)

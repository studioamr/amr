#!/usr/bin/env python3
"""Publica ELLA: m4a del set (ya masterizado, sin loudnorm), cantera.js, cortes standalone."""
import os, json, subprocess
import numpy as np, imageio_ffmpeg
from dream_core import ffmeter, master_file
from make_ella import SECTIONS, TMP, SPB, XF_BARS

FF = imageio_ffmpeg.get_ffmpeg_exe()
HERE = os.path.dirname(os.path.abspath(__file__))
SRC = os.path.join(HERE, 'masters', 'amr-ella.wav')
CUTS = ['NOMBRE', 'RISA', 'BAILE', 'SILENCIO', 'CAMPANAS', 'AMANECER']
...
if __name__ == '__main__':
    # 1. set completo
    m4a = os.path.join(HERE, 'audio', 'amr-ella.m4a')
    enc(SRC, m4a)
    ...
    meta = dict(id='amr-ella', title='ELLA', kicker='PARA ELLA · 0→100',
                tracks=len(titles), dur=round(acc, 1), titles=titles, offsets=offsets,
                file='audio/amr-ella.m4a', art='art/shots/shot-ella.svg', edition=12,
                peaks=peaks(m4a), bpm=125, key='A MIN')
    with open(os.path.join(HERE, 'ella.js'), 'w') as f:
        f.write('window.AMR_ELLA=' + json.dumps(meta) + ';')
    print('ella.js escrito', flush=True)
    ...
    for i, s in enumerate(SECTIONS):
        ...
        dst = os.path.join(HERE, 'audio', f'amr-ella-cut-{s["name"].lower()}.m4a')
        enc(mst, dst)
        os.remove(mst)

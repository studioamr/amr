#!/usr/bin/env python3
"""Publica COLIBRI: m4a del set (ya masterizado, sin loudnorm), cantera.js, cortes standalone."""
import os, json, subprocess
import numpy as np, imageio_ffmpeg
from dream_core import ffmeter, master_file
from make_colibri import SECTIONS, TMP, SPB, XF_BARS

FF = imageio_ffmpeg.get_ffmpeg_exe()
HERE = os.path.dirname(os.path.abspath(__file__))
SRC = os.path.join(HERE, 'masters', 'amr-colibri.wav')
CUTS = ['NECTAR', 'POLEN', 'VUELO', 'CENOTE', 'FIESTA', 'SELVA']
...
if __name__ == '__main__':
    # 1. set completo
    m4a = os.path.join(HERE, 'audio', 'amr-colibri.m4a')
    enc(SRC, m4a)
    ...
    meta = dict(id='amr-colibri', title='COLIBRÍ', kicker='TROPICAL SOLAR · 0→100',
                tracks=len(titles), dur=round(acc, 1), titles=titles, offsets=offsets,
                file='audio/amr-colibri.m4a', art='art/shots/shot-colibri.svg', edition=12,
                peaks=peaks(m4a), bpm=120, key='C MAJ')
    with open(os.path.join(HERE, 'colibri.js'), 'w') as f:
        f.write('window.AMR_COLIBRI=' + json.dumps(meta) + ';')
    print('colibri.js escrito', flush=True)
    ...
    for i, s in enumerate(SECTIONS):
        ...
        hist = master_file(secw, mst, target_i=-9.5, ceiling_db=-1.0)
        dst = os.path.join(HERE, 'audio', f'amr-colibri-cut-{s["name"].lower()}.m4a')
        enc(mst, dst)
        os.remove(mst)

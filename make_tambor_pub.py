#!/usr/bin/env python3
"""Publica TAMBOR: m4a del set (ya masterizado, sin loudnorm), cantera.js, cortes standalone."""
import os, json, subprocess
import numpy as np, imageio_ffmpeg
from dream_core import ffmeter, master_file
from make_tambor import SECTIONS, TMP, SPB, XF_BARS

FF = imageio_ffmpeg.get_ffmpeg_exe()
HERE = os.path.dirname(os.path.abspath(__file__))
SRC = os.path.join(HERE, 'masters', 'amr-tambor.wav')
CUTS = ['CUERO', 'PALMAS', 'CANTO', 'LUNA', 'RITUAL', 'TIERRA']
...
if __name__ == '__main__':
    # 1. set completo
    m4a = os.path.join(HERE, 'audio', 'amr-tambor.m4a')
    enc(SRC, m4a)
    ...
    meta = dict(id='amr-tambor', title='TAMBOR', kicker='AFRO MELODIC · 0→100',
                tracks=len(titles), dur=round(acc, 1), titles=titles, offsets=offsets,
                file='audio/amr-tambor.m4a', art='art/shots/shot-tambor.svg', edition=12,
                peaks=peaks(m4a), bpm=120, key='A MIN')
    with open(os.path.join(HERE, 'tambor.js'), 'w') as f:
        f.write('window.AMR_TAMBOR=' + json.dumps(meta) + ';')
    print('tambor.js escrito', flush=True)
    ...
    for i, s in enumerate(SECTIONS):
        ...
        dst = os.path.join(HERE, 'audio', f'amr-tambor-cut-{s["name"].lower()}.m4a')
        enc(mst, dst)
        os.remove(mst)
